...
faq:
  - question: "Go 微服务的分层架构怎么设计？"
    answer: "推荐四层：API 层（HTTP/gRPC 入口，做参数校验、鉴权、响应格式化）→ Handler 层（编排）→ Service 层（业务逻辑）→ Repository 层（数据访问）。职责单一、依赖单向，便于测试与扩展。"
  - question: "缓存一致性怎么保障？"
    answer: "采用\"删除缓存 + 事务联动\"策略：写操作在事务内更新数据库，同时通过 GORM AfterSave 钩子删除 Redis 缓存；若删缓存失败则触发事务回滚，从机制上杜绝脏读，保证缓存与数据库最终一致。"
  - question: "简历服务为什么适合 gRPC？"
    answer: "简历服务内部调用频繁、对延迟敏感，gRPC 基于 HTTP/2 + Protobuf 序列化，传输效率高、支持流式与双向通信，配合 K8s 服务发现和可观测性，非常适合微服务内部的高频数据交互。"
keywords: ["Go微服务", "分层设计", "缓存一致性", "简历服务"]
---
